...
class NeuralNet:
    def __init__(self, url, header=True, h1=4, h2=2):
        ...
        raw_input = pd.read_csv(url, header=None, names=names)
        # TODO: Remember to implement the preprocess method
        self.preprocess(raw_input)
    ...
    def preprocess(self, input_dataset):
        category = {"class": {"Iris-setosa": 0, "Iris-versicolor": 1, "Iris-virginica": 2}}
        input_dataset.replace(category, inplace=True)
        ncols = len(input_dataset.columns)
        nrows = len(input_dataset.index)
        self.X = preprocessing.normalize(input_dataset.iloc[:, 0:(ncols - 1)].values.reshape(nrows, ncols - 1))
        self.y = preprocessing.normalize(input_dataset.iloc[:, (ncols - 1)].values.reshape(nrows, 1))
        self.X, self.X_test, self.y, self.Y_test = train_test_split(self.X, self.y, test_size=0.25,
                                                                    random_state=np.random)

    # Below is the training function

    def train(self, max_iterations=1000, learning_rate=0.05, activation="tanh"):
    ...
    def backward_pass(self, out, activation):
        ...
        self.compute_input_layer_delta(activation)
    ...
    def compute_input_layer_delta(self, activation):
        delta_input_layer = np.multiply(self.__activation_derivative(self.X01, activation),
                                        self.delta12.dot(self.w01.T))
    ...
    def predict(self, activation):
        ...
        #         print(out)
        print("The total test error is " + str(np.sum(error)))
        #         print(self.Y_test)
        return 0
...
if __name__ == "__main__":
    ...
    #     neural_network.train(5000, 0.005, "sigmoid") #0.065
    #     testError = neural_network.predict("sigmoid") #0.0504
    neural_network.train(1000, 0.005, "tanh")  # 0.00568
    testError = neural_network.predict("tanh")  # 0.00580
#     neural_network.train(200, 0.015, "ReLu") #24.5
#     testError = neural_network.predict("ReLu") #25.5
